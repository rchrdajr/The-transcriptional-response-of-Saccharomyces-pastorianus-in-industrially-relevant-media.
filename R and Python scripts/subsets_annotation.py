...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 30 13:06:27 2024

@author: RichardReynolds
"""
...
## change directory
os.getcwd()
os.chdir("/Users/RichardReynolds/Desktop/unnorm_data_1")
#%%
##---------------------------------
...
for tempcond in temp_conditions:
    for cond in list_conditions:
        with open("analysis/fc2_aa_vs_"+str(cond)+"/subset_fc2_"+str(tempcond)+"aa_vs_"+str(tempcond)+str(cond)+".csv", "r") as data, open("analysis/fc2_aa_vs_"+str(cond)+"/subset_fc2_"+str(tempcond)+"aa_vs_"+str(tempcond)+str(cond)+"_annotated.csv", "w") as annot_output: #open file
            partheader = next(data)
            partheader = partheader.split("\n")[0]
            
            partheader = partheader.split(",")
            partheader = "\t".join(partheader)
            newHeader = partheader + "\t" + header
            print(partheader)
            print(header)
            annot_output.write(newHeader)
          #  print(newHeader)
            for line in data:
                line = line.split("\n")[0]
                line = line.replace('"','')
                ...
                gene = line[0]
                if gene in annotation_dico.keys():
                    annot = annotation_dico[gene]
                    #print(annot)
                else:
                    annot = "\n"
                newLine = "\t".join(line) + "\t" + annot 
                annot_output.write(newLine)
